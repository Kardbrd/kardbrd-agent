...
func (e Claude) CheckAuth(ctx context.Context) AuthStatus {
	...
	stdout, stderr, code, err := runCommand(ctx, Config{Timeout: e.timeout()}, "", []string{"claude", "auth", "status"}, "", "claude auth status timed out")
...
func (e Claude) Execute(ctx context.Context, req Request) Result {
	...
	stdout, stderr, code, err := runCommand(ctx, e.cfg, e.cwd(req), cmd, req.Prompt, "Claude execution timed out")
	emitChunks(stdout, "claude", req.OnChunk)
